...
describe('persist migration', () => {
  ...
  it('v7 drops the removed collections keys WITHOUT touching saved gradients', async () => {
    // Boards were deleted. A collection only ever held ids pointing into
    // `saved`, so dropping them must not cost anyone a palette — that is the
    // one genuinely risky part of removing the feature.
    localStorage.setItem(
      'palette-saved-gradients',
      JSON.stringify({
        state: {
          saved: [{ ...sampleGradient, id: 'keep-me', name: 'Keep Me' }],
          noiseEnabled: false,
          collections: [
            { id: 'c1', name: 'Kiln', createdAt: 1, gradientIds: ['keep-me'], levers: { temp: 50, depth: 50, char: 50 } },
          ],
    ...
    expect(useAppStore.getState().saved).toHaveLength(1)
    expect(useAppStore.getState().saved[0].id).toBe('keep-me')
    expect(state.collections).toBeUndefined()
    expect(state.activeCollectionId).toBeUndefined()
    // And the keys must not be written back out to localStorage.
    const raw = JSON.parse(localStorage.getItem('palette-saved-gradients') || '{}')
    expect(Object.keys(raw.state)).not.toContain('collections')
    expect(Object.keys(raw.state)).not.toContain('activeCollectionId')
  })
})
