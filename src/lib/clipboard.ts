 ...
 * verbatim; we escape the "]]>" terminator as "\\u005D\\u005D\\u003E" in the JSON string
 * to prevent XSS vulnerabilities if the SVG is opened in a browser. */
function embedPayload(svg: string, json: string): string {
  // Sanitize the JSON string to avoid closing the CDATA block prematurely.
