    ...
    debounceTimer.current = window.setTimeout(async () => {
      try {
        ...
        for (const word of terms) {
          // 🛡️ Sentinel: Sanitize user input to prevent PostgREST filter injection
          // and LIKE wildcard abuse. Stripping reserved characters ensures the
          // input is treated purely as a substring match, not as query syntax.
          const sanitizedWord = word.replace(/[,.%_]/g, '')
          if (!sanitizedWord) continue // skip if stripped to empty string
          ...
          if (familyKey) {
            ...
            const orString = `display_name.ilike.%${sanitizedWord}%,` + nouns.map(n => `display_name.ilike.%${n}%`).join(',')
            queryBuilder = queryBuilder.or(orString)
          } else {
            queryBuilder = queryBuilder.ilike('display_name', `%${sanitizedWord}%`)
          }
        }
