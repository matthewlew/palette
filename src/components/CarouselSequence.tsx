 ...
 * in, on the same cards — split into three tracks by what a slide actually
 * is, rather than one continuous row.
 *
 * A cover and a summary used to sit in the same track as the gradients,
 * distinguished only by a small badge on the card — easy to miss, and it read
 * as "gradient #1 of 9" rather than "the cover, plus 8 gradients". They are
 * not picks: a cover has no position of its own to drag to, and a summary
 * that isn't last isn't a summary. Separate, labelled tracks say that at a
 * glance instead of relying on someone reading a chip.
 *
 * Order and preview used to be two sections too, showing the same sequence
 * twice; they disagreed about what a "slide" was as soon as a cover was
 * involved. Merging them removes the translation step — you drag the thing
...
export function CarouselSequence({
  ...
  function renderItem(slide: CarouselSlide, i: number) {
    ...
    return (
      ...
      >
        <div className={styles.frame}>
          <SlideCanvas slide={slide} gradients={gradients} parts={parts} ratio={ratio} framed={framed} />
  ...
  const indexed = slides.map((slide, i) => ({ slide, i }))
  const cover = indexed.find(({ slide }) => slide.role === 'cover')
  const summary = indexed.find(({ slide }) => slide.role === 'summary')
  const body = indexed.filter(({ slide }) => slide.role === 'body')

  return (
    <div className={styles.tracks} data-testid="carousel-sequence">
      {cover && (
        <section className={styles.track} data-testid="track-cover">
          <h5 className={styles.trackLabel}>Cover</h5>
          <ol className={`${styles.sequence} ${styles.sequenceSingle}`}>{renderItem(cover.slide, cover.i)}</ol>
        </section>
      )}

      <section className={styles.track} data-testid="track-gradients">
        <h5 className={styles.trackLabel}>Gradients</h5>
        <ol className={styles.sequence} aria-label="Carousel slides">
          {body.map(({ slide, i }) => renderItem(slide, i))}
      ...
      {summary && (
        <section className={styles.track} data-testid="track-summary">
          <h5 className={styles.trackLabel}>Summary</h5>
          <ol className={`${styles.sequence} ${styles.sequenceSingle}`}>{renderItem(summary.slide, summary.i)}</ol>
        </section>
      )}
    </div>
  )
