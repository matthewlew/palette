...
export function Gallery({ onRiff, onImport, onStartType, onStartDrum, onViewerOpenChange, onSelectionActiveChange }: GalleryProps) {
  ...
  // Optimization: derived lists and Sets are wrapped in useMemo to prevent
  // expensive O(n) filtering and instantiations on every single render.
  // This is especially critical during drag-and-drop operations, where
  // hovering triggers a full re-render at 60fps.
  const filteredSaves = useMemo(
    () => saved.filter((gradient) => matchesFilters(gradient, typeFilter)),
    [saved, typeFilter]
  )
  const filtered = useMemo(
    () => (savesOrder === 'recent' ? byMostRecent(filteredSaves) : filteredSaves),
    [savesOrder, filteredSaves]
  )
  const filteredCommunity = useMemo(
    () => communityGradients.filter((gradient) => matchesFilters(gradient, typeFilter)),
    [communityGradients, typeFilter]
  )
  ...
  const filterPool = useMemo(
    () => (activeTab === 'community' ? communityGradients : saved),
    [activeTab, communityGradients, saved]
  )
  ...
  const availableTypeChips = useMemo(
    () =>
      TYPE_CHIPS.map(({ type, label }) => ({
        type,
        label,
        count: filterPool.filter((g) => g.type === type).length,
  ...
  const searchFlat = useMemo(
    () => (searchResults ? [...searchResults.mine, ...searchResults.community] : null),
    [searchResults]
  )
  const currentViewGradients = useMemo(
    () => searchFlat ?? (activeTab === 'community' ? filteredCommunity : filtered),
    [searchFlat, activeTab, filteredCommunity, filtered]
  )
