...
export function Gallery({ onRiff, onImport, onStartType, onStartDrum, onViewerOpenChange, onSelectionActiveChange }: GalleryProps) {
  ...
  const orderKeyArray = useMemo(() => currentViewGradients.map((g) => g.id).join(','), [currentViewGradients])
  ...
  const orderKey = useMemo(() => `${orderKeyArray}-${activeTab}`, [orderKeyArray, activeTab])
