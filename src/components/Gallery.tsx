...
export function Gallery({ onRiff, onImport, onStartType, onStartDrum, onViewerOpenChange, onSelectionActiveChange }: GalleryProps) {
  ...
  // ⚡ Bolt: Memoize filtered arrays and derived sets to avoid O(N) calculations
  // during high-frequency drag-and-drop operations, which trigger state updates
  // like `setDragOverId` at 60fps, causing severe performance drops otherwise.
  ...
  // ⚡ Bolt: Memoize nested filter maps to avoid expensive looping on every render
  ...
  // ⚡ Bolt: Memoize the Set creation to avoid object allocation and copying per render
  ...
  // ⚡ Bolt: Memoize the likes API object to prevent downstream components (e.g. Tile)
  // from breaking React.memo and re-rendering on parent hover/drag state changes.
