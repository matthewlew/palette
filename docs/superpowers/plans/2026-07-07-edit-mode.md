...
attribute. (An earlier draft of this note also proposed a conic-gradient swatch
preview per row; the code sample below never actually included one, and given
`BlockWheel` is already an explicitly temporary stand-in for a future real SVG
wheel, that's been dropped rather than added after the fact — not worth the
extra surface area on a component already slated for replacement.) A true SVG
wheel with angular drag can be a follow-up visual polish task once this is
functionally working end-to-end.
